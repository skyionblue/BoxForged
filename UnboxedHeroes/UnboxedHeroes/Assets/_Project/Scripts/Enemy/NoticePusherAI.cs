...
namespace Boxhead.Enemy
{
    ...
    public class NoticePusherAI : MonoBehaviour
    {
        [Header("Detection")]
        ...
        [Header("Movement")]
        [SerializeField] private float _patrolSpeed = 1.4f;
        ...
        private void Update()
        {
            ...
            switch (_state)
            {
                ...
                case State.Chase:
                    if (distToPlayer > _detectionRadius * 1.5f)
                        _state = State.Patrol;
                    else if (distToPlayer <= _attackRadius)
                        _state = State.Attack;
                    break;

                case State.Attack:
                    if (distToPlayer > _attackRadius * 1.5f)
                        _state = State.Chase;
                    break;
            }
            ...
            {
                var drop = new Vector3(0f, -9.81f * Time.deltaTime, 0f);
                _controller.Move(drop);
            }
        }
        ...
        private void DoPatrol()
        {
            ...
            Vector3 target = _patrolCenter + new Vector3(Mathf.Sin(rad) * _patrolRadius, 0f, Mathf.Cos(rad) * _patrolRadius);
