...
ADR-0002's "one scene per room" corollary is superseded **for World 1 only**. `CulDeSac_WildWestCity.unity` is one continuous street played start to finish, not a series of scene loads. This did not require replacing `RoomManager`/`RoomDataSO` — `RoomManager` already models an ordered list of encounter zones inside a single scene (`_rooms`, `RoomTrigger.OnTriggerEnter` → `OnRoomEntered(index)`), it had just never been used that way. Three `RoomDataSO` zones (Arrival, Ambush Alley, boss-only Showdown Circle) are appended via `LevelBuilder.RoomData` as before; what changed is `GameManager` — `RoomManager.HasZoneAfterCurrent` now lets a room-clear advance to the next zone in-scene instead of always calling `LoadNextRoom()`/`SceneManager.LoadScene`. A scene-local `WildWestCityZoneDirector` handles the one piece of behavior specific to this layout (clearing two covered-wagon props to open the boss's fight space on zone-2 entry). See ADR-0004 for the full zone geometry, the gate/NavMesh-carving pattern (`RoomGate` now also toggles a `NavMeshObstacle`, not just its `Collider`/`Renderer`), and the boundary-wall pattern that seals the street against flanking the gates.

### 4.2.2 Single continuous scene is now the default — ADR-0005

**ADR-0004's "World 1 only" scoping is superseded by [ADR-0005](adr/0005-world2-single-continuous-scene.md) (2026-08-31).** World 2 (`Backyard_Dojo.unity`, three zones) uses the same pattern, and single-continuous-scene is now the **default for team-built worlds**. ADR-0002's "one scene per room" corollary is retired as the default; its `RoomDataSO`/`LevelBuilder` data-driven encounter layer (§4.2 above) is preserved unchanged and remains the load-bearing contract.

The decisive fact is that the room-per-scene path is **currently dead code with no valid targets**: `RandomRoomPool` is empty (`GameManager.cs:52`), `LoadNextRoom()`'s exhausted-queue branch logs an error rather than loading (`:607-615`), every per-room scene was deleted in commit `84a3a44e`, and `ZoneStartScene[1]` names a scene that has never existed (see §5 divergence 11). It also cannot reach the shop screen, because reward routing keys off in-scene zone index and a per-room `RoomManager` always has one room at index 0.

The scene-load machinery (`RandomRoomPool`, `InitRoomQueue`, `s_roomQueue`, `CaptureLoadoutForTransition`, `LoadNextRoom`, `ZoneStartScene`) is **kept, not deleted** — a future world may legitimately need it (too large to hold resident, a real interior, a per-room lighting environment). It must be re-qualified before use, and such a world needs its own ADR. Both shipping worlds are single-scene, so it currently has zero users.

`WildWestCityZoneDirector` is generalized to a reusable **`ZoneDirector`** under ADR-0005 §5 (all four of its serialized fields were already scene-agnostic data); per-world one-off staging stays in a small scene-local script subscribing to `RoomManager.OnRoomActivated` / `GameManager.OnRoomClearScreenShown`.
...
| 11 | ~~**Three Build Settings scenes do not exist on disk**~~ — **FIXED 2026-08-27**, dead entries removed from `EditorBuildSettings.asset` (see `docs/BACKLOG.md` B11). ~~`GameManager.cs:34` zone 1 (`TownSquare_Room1`) is unreachable~~ — **FIXED 2026-08-31** ([ADR-0005](adr/0005-world2-single-continuous-scene.md) §7): `ZoneStartScene[1]` now points at `Backyard_Dojo`, a real scene registered in Build Settings; `ZoneIndexByScene["Backyard_Dojo"] = 1` added alongside it | `Core/GameManager.cs` |
| 12 | **No test infrastructure** — one trivial asmdef, zero tests, DoD unsatisfiable | |
| 13 | **Three navmeshes exist per `LevelBuilder` scene, only one ships.** The committed Editor `Scenes/<Scene>/NavMesh.asset` and `NavigationStatic` GameObject flags are Edit-mode tooling only — `LevelBuilder.BuildNavMeshDeferred()` discards them (`NavMesh.RemoveAllNavMeshData()`) and rebakes at runtime from live colliders (`NavMeshCollectGeometry.PhysicsColliders`) on every scene load, in **every** `LevelBuilder` scene (World 1 and World 2 both). **The runtime bake is authoritative — any navmesh claim must say which bake it measured.** A verified device/Editor split within the runtime bake itself was also found and fixed (B145): a non-convex `MeshCollider` whose source mesh isn't Read/Write-enabled bakes fine in Editor Play Mode (every mesh is CPU-readable there) but is silently dropped in a real device build | `Systems/LevelBuilder.cs` (`BuildNavMeshDeferred`); `docs/BACKLOG.md` B127/B133/B143/B145 |
