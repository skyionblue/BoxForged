...
**Status:** **Submitted to Apple for review, 2026-09-10** (owner-performed, App Store Connect). Screenshots (iPhone + 13" iPad) were added and the app was submitted. This section of the document is now historical for iOS — treat §2/§3's copy and rating answers as what was actually submitted, not a draft, unless the owner says otherwise. Google Play submission has not happened yet — the rest of this document (Android-relevant sections) is still draft/prep only.
...
- ~~Screenshots~~ — **DONE for iOS, submitted 2026-09-10.** Five real Play Mode captures (no physical device needed — Unity's Game View renders at any target resolution) at exact required dimensions: `world1-culdesac-gameplay.png`, `world2-backyard-dojo-gameplay.png`, `world2-grasscutter-boss.png` (all 2778×1284, iPhone), `ipad-culdesac-gameplay.png`, `ipad-spincycle-boss.png` (both 2732×2048, 13"/12.9" iPad — required once App Store Connect flagged the app as iPad-capable). All in `docs/media/store-screenshots/`. Google Play screenshots not yet done — Android has no submission track started at all (separate item, see `docs/SPRINT.md`).
- **App preview video** (15–30s, per the existing `docs/media/social-launch-playbook.md` checklist) — needs real gameplay footage. Not required for the iOS submission that already went in (Apple didn't block on it), still worth having for the listing.
- ~~Final owner read-through of §2's copy and §3's rating table before either goes into a store console~~ — **implicitly done**: the owner submitted the app using this document's copy/rating answers as-is. If anything here turns out wrong post-submission, it needs an update in App Store Connect directly, not just in this doc.
