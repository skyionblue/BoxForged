...
| [0008](adr/0008-boss-intro-camera-authored-vantage.md) | Boss-intro cameras use an authored vantage point, and three invariants that make the shot provable | **Accepted**, amended three times (2026-09-01 ×2, 2026-09-02 ×1) | 2026-09-01 |
| [0009](adr/0009-srp-batcher-and-the-draw-call-budget.md) | The SRP Batcher is engaged, it preempts GPU instancing, and TDD §3.2's `< 100 draw calls` budget is not reachable as written | **Accepted (Option A)**, pending one required on-device `SetPass Calls Count` reading (§7) | 2026-09-10 |

ADR-0001–0003 await owner approval. ADR-0004 was accepted 2026-08-26 once the owner resolved every question it had left open; it is the implementation spec for the city scene but does not by itself authorize a commit. ADR-0005 was accepted 2026-08-31 with the owner having explicitly delegated the scene-architecture choice to `technical-director`; its open questions are content decisions and do not block scaffolding. ADR-0006 amends ADR-0005 §3/§4 and restates `TECHNICAL_DESIGN.md` §6.4 after the owner playtested World 2's built geometry. ADR-0007 extends ADR-0003 with a second telegraph geometry and is what satisfies ADR-0006 §1.3's arena-size condition — **note that it extends a decision this table still records as Proposed.** ADR-0008 adds a project-wide contract for boss-intro cinematic cameras. Its first version claimed to **enforce** ADR-0006 §1.1; it was amended the same day after the owner confirmed the cherry tree's north-rim position is an intentional design decision, so **ADR-0006 §1.1 is superseded for that placement by owner authority** and ADR-0008's staging was re-derived with the tree pinned. §1.1's reasoning is preserved in place, annotated.

**ADR-0009 (2026-09-10) is the first ADR raised to *retract* a finding rather than propose a design, and it needs an owner decision before anything acts on it.** It establishes, by measurement, that Unity `6000.5.3f1`'s `SRP Batcher Draw Calls Count` profiler counter is broken — it reads 0 whether the SRP Batcher is on or off — so `BACKLOG` B112's, the World-1 recapture's and B132's three "independent" on-device confirmations that the SRP Batcher contributes nothing are **one measurement error repeated three times**. The batcher is engaged and halves SetPass calls (44 on, 85 off). Two live consequences: **ADR-0005 §3's grant condition is unsatisfiable as literally worded** and is restated in ADR-0009 §4 (after which it is satisfied, not failed); and **TDD §3.2's `< 100 draw calls` budget is not reachable while the SRP Batcher is enabled**, because the batcher preempts GPU instancing, which is the only mechanism that reduces the count. ADR-0009 §5 puts two mutually exclusive options to the owner — keep the batcher and budget SetPass calls instead (recommended, zero change), or disable it project-wide and chase the literal count via instancing (measured cost: SetPass roughly doubles). Until the owner picks, **no rendering setting should be changed and no scene should be failed against the `< 100` line.** Also note ADR-0006 §5.2's `BD-01-Long` refund loses its performance justification under this measurement (the 47-module stockade costs 10 draw calls, not ~47) and must be re-justified on art merit.

> **Status conflict, surfaced not resolved (2026-08-25; still open, evidence extended 2026-09-02):** this table lists 0001–0003 as Proposed, but `adr/0002-full-scene-rebuild.md`'s own header says **Accepted**, `docs/TECHNICAL_DESIGN.md` §11 states outright *"ADRs 0001–0003 are approved"*, and `docs/ROADMAP.md` records production as authorized on 2026-08-19. Sprint 0 shipped against 0001/0003, and four later ADRs (0005–0008) now extend or amend them — ADR-0007 in particular extends a decision this table still records as Proposed. This table is now the only source claiming Proposed. Owner should confirm which is correct and the losing source should be corrected.
...
*Accepted 2026-09-01, **amended the same day** after an owner correction. The summary below is of the amended ADR.*

Two attempts at the Grasscutter's intro shot both failed review by clipping into different geometry. The first version of this ADR concluded the cause was the cherry tree sitting 7.53 m off the Blossom Court's centre, and moved it back. **The owner corrected that premise — the north-rim position is intentional** (*"I moved the tree to the back of the area because it made more sense back there"*) — so **ADR-0006 §1.1 is superseded for this placement by owner authority**, §1.1's reasoning stays on the record annotated, and the whole staging was re-derived with the tree as a fixed constraint.

**Three structural findings, all re-verified against the live scene rather than carried over.** (1) **No `_introCamDistance` value could ever have worked.** `ComputeIntroCamPosition` retreats *away from the player*, so the camera's radius from the arena centre is `r_boss + D` — 22.0 m in a 10.0 m arena. The retreat rule is an implementation detail, not a GDD requirement. (2) The `CourtMakiwara` clip found in review is a **false positive** — all six court props are in `ZoneDirector._clearOnBossZone` and are deactivated *before* the boss activates. (3) **`Renderer.bounds` is not a size measurement on this project** and has now published a wrong number twice: it is a world AABB, inflated by rig rotation on skinned meshes and by `[ENV - Static]`'s 45° yaw on environment meshes. Measured from mesh vertices, the Grasscutter is **3.40 m** tall, not the 4.70 m B119 reported, and SpinCycle is 4.25 m — they do not match (`docs/BACKLOG.md` B123).

**Decision.** The tree does not move. The boss is staged at the arena's north-**west**, design `(−6.5, 0, 58.0)` = world `(36.416, 0, 45.608)`, r = 7.16 m of 10.0 with 1.149 m of blade clearance to the wall. The vantage becomes an **authored `Transform`** (`_introCamAnchor`, with the old computation kept as a warned fallback) at world `(36.416, 1.800, 39.300)` — placed so the camera→boss axis lies along **world +Z**, matching `pfb_CM_FollowCam`'s locked yaw exactly. With `_introCamFoV` set to 45 (the gameplay lens), the Phase D handoff becomes a pure pitch-and-position cut: zero yaw rotation, zero focal-length change.

**Three invariants replace the tuning.** **I1:** camera and both look targets are interior points of the arena — the polygon is convex, so no sightline can cross the boundary at any approach bearing. **I2:** `_introTickOverRange` > `_introTriggerRange` > `distance(anchor, boss)`, so a player triggering from further out is provably behind or beside the camera. **I3 (new):** during the tree shot the boss is **entirely below the opaque ground plane**, so it cannot be spoiled in that shot at any aspect ratio — replacing an angular-separation margin that, with the tree pinned, came down to 0.1° at 20:9. Verified: 7.49 m wall clearance, 0 of 448 boss-silhouette rays blocked by `Building` physics *or* the tree's voxelised mesh, 4.097 m from any tree geometry to the camera→boss segment, and **0 of 64 800** trigger-sphere samples in frame or occluding at 21:9.

**Framing restores a World 1 pattern that the port dropped.** Phase C aimed at the boss's **feet**; `SpinCycleAI` has carried `_introCamLookHeight = 2.0f` all along. New `_introBossLookHeight = 1.70` (50% of the measured 3.40 m) and `_introTreeLookHeight = 4.50` — the tree mesh's **area-weighted centroid**, not its `Renderer.bounds.center.y` of 3.810, which sits in the trunk. The boss fills **65.1%** of frame height, inside the 63–68% band SpinCycle's playtested reveal sets.

**Two traps the re-measurement exposed, both now mandatory reading before any re-verification.** The cherry tree's **canopy has no collider** — only the 0.35 m trunk capsule is on the `Building` layer — so a `Physics.Linecast` clearance sweep passes through 8.8 m of foliage and reports the shot clean (`docs/BACKLOG.md` B126). And `Backyard_Dojo.unity` has **no baked NavMesh at all** (`m_NavMeshData: {fileID: 0}`), which blocks implementation and contradicts B116's reported `PathComplete` validations (`docs/BACKLOG.md` B122).

**The prefab is what ships.** `pfb_enemy_grasscutter.prefab` carries `_introCamDistance: 14`, which shadows the class default — an earlier edit to the C# default changed nothing that ships. Every value must be written into the prefab and the YAML re-read to confirm.

Rejected: tuning the distance again (provably impossible); the boss at arena centre (trades one violation for three); re-anchoring the computation on the arena centre (close, but hides the shot in arithmetic and cannot express the lateral offset the tree needs); a wall-avoidance raycast clamp (more code, unpreviewable, treats the symptom); two vantages (unnecessary at 46.3° separation); moving the tree instead (inverts an ADR to accommodate a defect in implementing it).

**World 1 is re-read and not changed** — SpinCycle satisfies I1 by construction and already has the look-height field; its intro is activation-gated, so I2 does not apply. Implementation is `docs/BACKLOG.md` **B119** (`unity-gameplay-engineer`); the residual gap that no boss intro locks player movement is **B120**.
