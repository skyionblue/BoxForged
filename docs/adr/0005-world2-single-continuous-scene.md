...
| Combat radius, per zone | **≤ 9 m** | TDD §6.4, derived from the camera's 16.8 m visible width |
| Boss arena minimum clear circle | **r ≥ 8.5 m** | World 1's measured 8.44 m holds SpinCycle. See §4 — this one runs the other way round for World 2 |
...
|---|---|---|
| Boss arena minimum clear circle | **r ≥ 8.5 m** (~17 m across) | Matches the camera's 16.8 m visible lateral width and World 1's measured, playtested 8.44 m |
| Phase-2 Spin-Dash travel | **≤ 8 m** | Must start and end inside the arena. SpinCycle's charge is 4.8 m; 8 m is generous, and the blueprint's *"straight-line Spin-Dashes across the court"* must mean across an 17 m court, not a 36 m one |
...
**Correction of record:** the blueprint's *"~36 m circular sparring court"* (`unity-blueprint.md` §Boss Room) predates ADR-0001 and violates TDD §6.4's ≤ 9 m combat radius by a factor of two. Over half that fight would happen off screen. The Blossom Court is ~17–20 m across with the cherry tree at centre, not 36 m.
...
1. **Author the ENV root at yaw 0.** World 1's `[ENV - Static]` is rotated **45°**, so every design coordinate needs a local↔world transform, `Renderer.bounds` inflates footprints badly, and raw world X/Z say nothing about the level (ADR-0004 §0; `.claude/agent-memory/technical-director/reference_measuring_city_scene.md`). Build the yard axis-aligned, set `LevelBuilder._cameraYawDegrees = 0`, and design coordinates **are** world coordinates. This is the single largest avoidable tax on World 1 and it buys nothing.
...
- World 1's eight authoring lessons (§6) are paid forward instead of re-learned — most valuably the yaw-0 ENV root, which removes World 1's coordinate-transform tax entirely.
...
- **The boss arena is budgeted before the boss exists**, which inverts World 1's much safer measure-then-place order. If `GrasscutterAI` cannot be authored inside §4's envelope, the arena has to grow, and at ~17 m across it is already at the camera's visible width. Surface that early, not during boss tuning.
...
3. **Large-agent NavMesh:** second agent type, or cap boss agent radius at 0.5? Recommendation: cap the radius. Blocking for the Grasscutter (§4).
