...
- **The perimeter is diegetic.** World 1 needed `StreetBoundary_West`/`_East` — two invisible retrofit colliders added after a physics flood-fill found the player could walk around the outside of the entire building row and skip the gates (ADR-0004 §1, second and third fix passes, `docs/BACKLOG.md` B99). The dojo's bamboo stockade **is** the boundary, authored from the start, visible, and part of the art.
  ...
  > **Amendment, 2026-09-11 — the "visible" half of this is superseded by owner direction.** The owner asked for World 2's walls to be removed and replaced with invisible boundaries "same way the first level is". All 47 `BD01_WallModule` objects had their `MeshRenderer`/`MeshFilter` stripped; all 47 `BoxCollider`s remain. So the **structural** claim above still holds in full — the perimeter is still authored from the start, still sealed, still physically continuous, and `LevelBuilder`'s bake reads physics colliders rather than render meshes, so nothing about navmesh or collision changed. What no longer holds is *diegetic and part of the art*: the boundary is now invisible in both worlds, and World 2's perimeter is a retrofit-shaped thing by appearance even though it is not one by construction. The 7 return walls at the z = 17 and z = 45 chokepoints were kept visible in a first pass and then removed as well on owner review ("I don't want the choke points they look odd"), so **World 2's zone gates now have no visual representation whatsoever** — §6 item 4's concern about silent gate/trigger failures gets sharper, because there is no longer any on-screen artefact that would make a mis-sized gate obvious to a playtester. A gate visual (not a wall) is the natural follow-up if this reads badly in play. §6 item 3's sealed-perimeter requirement is unaffected and still satisfied; §6 item 7's "let the stockade do the work" is likewise unaffected. See `docs/CHANGELOG.md` 2026-09-11 and `docs/BACKLOG.md` B146.
...
| Distinct ENV materials in the scene | **≤ 20** | New. **Condition discharged 2026-09-10 (ADR-0009):** the "SRP Batcher contributed 0" reading (B112) was a broken profiler counter, not a real defect — the batcher is genuinely engaged (44 SetPass calls with it on vs. 85 off, measured live). This row's original condition ("verify a non-zero SRP Batcher or Instanced draw-call count on device") is unsatisfiable as worded and is restated per ADR-0009 §4: verify `SetPass Calls Count` is materially below the renderer count instead — satisfied (44 vs. ~87 visible renderers) |
