...
>
> **B126 RESOLVED — 2026-09-09, owner decision, no changes to shipping World 1/World 2 content.** The envelope is amended to match the built asset rather than the asset being re-authored: canopy radius and underside in the table below are superseded by the built ellipse (half-axes 4.4 × 2.5 m) and underside (y ≈ 1.9 m) measured above. Separately, **the canopy will not get a collider.** The resulting false-clear gap in physics-based clearance sweeps (this ADR's §Validation, M2, `Systems/BuildingOcclusionFader.cs`) is an accepted, known limitation, not a defect to be fixed — any future clearance check against this tree must account for it manually (e.g. the 0.2 m voxelisation approach ADR-0008 §Validation 4 already uses) rather than trusting a raw `Physics.Raycast`.
