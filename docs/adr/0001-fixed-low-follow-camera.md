...
| **Tall props need to sit near where the player actually stands — "distant backdrop" placement doesn't work on this rig** | Confirmed 2026-08-19 building World 1 Room 1 (`docs/BACKLOG.md` B57) | The upper view-frustum edge sits at a fixed `pitch − vFOV/2 = 13.5°` below horizontal. For a point at height `h` to stay in frame, camera-to-point depth must be `≤ (cameraHeight − h) / tan(13.5°)`. Concretely: a 7.18m-tall saloon facade's door/sign (at ~3.5m) only clears the top of frame when the player is within ~3.2m of it along the camera's fixed depth axis — moving a tall prop further from where combat actually happens (e.g. to a "backdrop" wall) pushes its identifying features (signage, doors, windows) above `viewport.y = 1.0`, off-screen, even though the base of the prop is still visible and the camera-clearance validator still passes (clearance checks geometry occlusion, not frame composition). **Rule of thumb for future rooms:** any prop taller than roughly the camera's own height (~6.35m at the base rig) needs to be placed within a few metres of where the player will actually be standing when they'd look at it — not at the edges of a room as scenery. This is a composition constraint, not a bug, and won't be caught by the clearance validator; verify tall-prop placement with `Camera.main.WorldToViewportPoint` at a realistic combat position, not just visual inspection from an arbitrary editor angle |
