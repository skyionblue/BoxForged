...
**ADR-0004 was accepted 2026-08-26** once the owner resolved every question it had left open (three zones, mixed roster in zones 0–1, boss-only zone 2, no ground expansion). It is now the implementation spec. Implementation itself — authoring the three `RoomDataSO`s and the merged drop table, wiring `LevelBuilder`/`RoomManager`/triggers/gates into the scene, the new zone-director script, and the supporting `GameManager` change — is the next task and still requires the owner's normal commit approval.
