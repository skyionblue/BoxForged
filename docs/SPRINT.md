...
**Status: AUTHORIZED, in progress.** Owner said "I would like to get this game deployed for Android, and iOS" (2026-09-08), immediately after closing out Sprint 1's crash-recovery cleanup commit (`ac1fa96e`) on `feature/sprint-0-foundation-rebuild`. New branch: `feature/mobile-release-readiness`. Sprint 1 (below) remains open in parallel — additive scope, not a supersession. **iOS has since gone past TestFlight-only scope: the app was submitted to Apple for App Store review on 2026-09-10** (see "WHERE WE LEFT OFF" below) — this sprint's original "TestFlight, not full submission" framing is stale for iOS specifically; Android remains TestFlight-equivalent informal testing only, no submission.
...
**WHERE WE LEFT OFF (2026-09-10, later same day):** app submitted to Apple for App Store review. Blocked initially on a missing required screenshot for 13" iPad displays (App Store Connect flagged the app as iPad-capable, which the existing iPhone-only screenshot set didn't satisfy) — resolved by capturing iPad-resolution screenshots (2732×2048) directly in the Unity Editor via Play Mode, no physical iPad needed (Game View renders at any target resolution; captures were cropped to each store's exact required pixel dimensions using `sips`, verified visually for HUD clipping before use — one iPhone-aspect capture of World 1 had to be redone live rather than cropped, since the HUD margins were too tight to crop safely at that aspect). Five screenshots now in `docs/media/store-screenshots/`, submission is in Apple's review queue. See `docs/STORE_LISTING.md` §7 for the full asset list. **Not yet done for iOS: nothing — this was the last blocking item.** Android is unaffected/untouched by this — still no Play Console submission started.

**WHERE WE LEFT OFF (2026-09-10, earlier in the day):** owner asked to close out B132, B133/B134, and B105 before ending this sprint, plus the remaining process-debt item. All four resolved: B105 fixed (owner decided win-over-death priority when both happen in the same window); B133/B134 resolved (NavMesh modifiers confirmed working under the runtime bake, B127 closed as moot, B134 downgraded — negligible cost); B132 **retracted**, not just fixed — the "SRP Batcher contributes zero" finding that drove two weeks of investigation turned out to be a broken profiler counter, not a real defect (see ADR-0009). `docs/KNOWN_ISSUES.md`, `docs/CHANGELOG.md`, `docs/AI_CONTEXT.md` created, closing the last process-debt item. **New open items from this session:** ADR-0009 needs an owner decision (SRP Batcher rendering-budget strategy — see below), and B144 (geometry density, not B132's wall-module theory) is now the real live rendering item. B142 (intermittent no-win-screen) recurred again via owner report, still blocked on a device console capture.
...
- ~~**B133/B134**~~ — World 2's NavMesh findings — **RESOLVED 2026-09-10.** B133: verified live that `NavMeshModifier` exclusions DO work correctly under World 2's actual runtime bake (unlike the Editor's legacy bake) — B127 closes as moot, no owner decision needed, the system already does what its author intended. B134: the runtime bake costs ~6ms against a 500ms scene-start budget — negligible, downgraded P1→P3, B128's bounds-volume fix deliberately not implemented (the justification for it is gone, and it turned out to be a bigger shared-system change than assumed).
- ~~**B145**~~ — `KoiPond`'s navmesh contribution silently differed between Editor and a real device build (a Read/Write flag gap on its FBX importer) — **FIXED 2026-09-10.** Ticked Read/Write Enabled on `Koi_pond_basin.fbx`; verified live (the "does not allow read access" warning is gone). Checked `CulDeSac_WildWestCity.unity` too — World 1 has no instance of this defect class (its only `MeshCollider` is a built-in primitive, not an FBX asset).
- ~~**B143**~~ — the committed Editor `NavMesh.asset`/`NavigationStatic` flags are dead weight at runtime in both worlds — **DOCUMENTED 2026-09-10**, added as a named architectural divergence in `docs/ARCHITECTURE.md` §5 rather than left as tribal knowledge. No code change needed.
...
- ~~**B144**~~ — Dojo ENV props are geometry-dense with no LODs (the stepping-stone prop, not B132's wall-module theory) — **RESOLVED 2026-09-10.** Decimation was tried and abandoned as infeasible (source mesh has 467 non-manifold edges/83 holes; floor is ~850 tris before visible breakage, well short of target). Owner removed the 32 stepping-stone props (plus 4 unrelated placeholder objects) directly in the Editor. Real measured result: zone 0 triangles 255,359→**140,677** (−45%), draw calls 163→**97** (−40%). World 2 is now well inside budget. See `docs/BACKLOG.md` for the live-Editor-collision incident this caused mid-task — worth reading before running another asset-engineer pass while a human might be editing the same scene.
